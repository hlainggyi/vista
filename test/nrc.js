...
MMNRC.prototype = {
  inti: function(nrc) {
    if ((this.match = regx_eng.exec(nrc))) {
      ...
      if (this.dist.length === 3) this.inCompleteInfo = true;
      return this;
...
MMNRC.prototype.getFormat = function(lang) {
  if (lang && lang === "mm" && !this.inCompleteInfo) {
    MMNRC.toMyaNum(this.state) +
      "/" +
      MMNRC.convDistrict(this.dist) +
      "(" +
      NAING_MM +
      ")" +
      MMNRC.toMyaNum(this.num);
  } else {
...
MMNRC.convDistrict = function(dist) {
  ...
  for (var i = 0; i < dist.length; i++) {
    if (!CHARACTERS[dist[i]]) return null;
...
MMNRC.formatConvert = function(nrc) {
  ...
  if ((_match = regx_eng.exec(nrc))) {
    ...
    // 3 Characters Districts are not compete and can"t be generate Full Format
    if (_res.dist.length === 3) console.warn("Incomplete format!");
    return _res;
